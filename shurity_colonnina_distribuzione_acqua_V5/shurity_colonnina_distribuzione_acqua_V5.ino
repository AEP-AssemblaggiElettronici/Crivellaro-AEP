...
void aggiorna_display_allarmi() {
  ...
  safe_lvgl_operation([&]() {
    ...
    if (giorni_installazione_filtro == 0) {
      lv_label_set_text(label_stato_allarme, "?? Filtro non configurato");
      lv_obj_set_style_text_color(label_stato_allarme, lv_color_hex(0x888888), 0);
      return;
    }
...
// Wrapper thread-safe per cambio schermata con debounce
bool safe_screen_transition(lv_obj_t *target_screen, lv_scr_load_anim_t anim_type, uint32_t time, bool auto_del) {
  unsigned long now = millis();

  // Debounce: ignora se troppo presto dall'ultima transizione
  ...
  safe_lvgl_operation([&]() {
    ...
    lv_obj_add_event_cb(
      btn_salva, [](lv_event_t *e) {
        safe_screen_transition(schermo2, LV_SCR_LOAD_ANIM_FADE_IN, 150, false);
        //enable_auto_return();  // AGGIUNGI QUESTA RIGA
      },
      LV_EVENT_CLICKED, NULL);
...
void callback_bottone_impostazioni_allarme(lv_event_t *evento) {
  ...
  safe_lvgl_operation([&]() {
    ...
    disable_auto_return();  // NON tornare indietro automaticamente
    lv_obj_add_event_cb(schermo_allarmi, touch_event_cb, LV_EVENT_PRESSED, NULL);

    // ============================================
    // CONTENITORE PRINCIPALE (ALTEZZA AUMENTATA)
    // ============================================
    lv_obj_t *container_principale = lv_obj_create(schermo_allarmi);
    lv_obj_set_size(container_principale, 700, 260);  // Era 280, ridotto
    lv_obj_align(container_principale, LV_ALIGN_CENTER, 0, 50);
    ...
    lv_obj_set_style_pad_all(container_principale, 5, 0);  // Era 10, ridotto a 5

    // ============================================
    // RIGA 1: ALLARMI ATTIVI
    // ============================================
    lv_obj_t *riga1 = lv_obj_create(container_principale);
    lv_obj_set_size(riga1, 690, 45);  // Era 680x50, ridotto
    lv_obj_align(riga1, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_bg_color(riga1, lv_color_hex(0xF8F9FA), 0);
    lv_obj_set_style_border_width(riga1, 0, 0);
    lv_obj_set_style_radius(riga1, 5, 0);
    lv_obj_set_style_pad_all(riga1, 0, 0);  // AGGIUNGI: rimuovi padding interno

    lv_obj_t *label_switch = lv_label_create(riga1);
    lv_label_set_text(label_switch, "Allarmi attivi:");
    lv_obj_align(label_switch, LV_ALIGN_LEFT_MID, 20, 0);
    lv_obj_set_style_text_font(label_switch, &lv_font_montserrat_16, 0);

    switch_allarmi_attivi = lv_switch_create(riga1);
    lv_obj_align(switch_allarmi_attivi, LV_ALIGN_RIGHT_MID, -20, 0);
    ...
    // ============================================
    // RIGA 2: GIORNI PREAVVISO
    // ============================================
    lv_obj_t *riga2 = lv_obj_create(container_principale);
    lv_obj_set_size(riga2, 690, 45);               // Ridotto
    lv_obj_align(riga2, LV_ALIGN_TOP_MID, 0, 48);  // Era 55, gap ridotto a 3px
    lv_obj_set_style_bg_color(riga2, lv_color_white(), 0);
    lv_obj_set_style_border_width(riga2, 0, 0);
    lv_obj_set_style_radius(riga2, 5, 0);
    lv_obj_set_style_pad_all(riga2, 0, 0);

    lv_obj_t *label_preavviso_text = lv_label_create(riga2);
    lv_label_set_text(label_preavviso_text, "Giorni preavviso:");
    lv_obj_align(label_preavviso_text, LV_ALIGN_LEFT_MID, 20, 0);
    lv_obj_set_style_text_font(label_preavviso_text, &lv_font_montserrat_16, 0);

    slider_giorni_preavviso = lv_slider_create(riga2);
    lv_slider_set_range(slider_giorni_preavviso, 1, 30);
    lv_obj_set_width(slider_giorni_preavviso, 220);
    lv_obj_set_height(slider_giorni_preavviso, 20);
    lv_obj_align(slider_giorni_preavviso, LV_ALIGN_RIGHT_MID, -140, 0);
    ...
    lv_obj_t *btn_minus_preavviso, *btn_plus_preavviso;
    crea_bottoni_plus_minus_estremi(riga2, slider_giorni_preavviso, &btn_plus_preavviso, &btn_minus_preavviso);
    ...
    label_giorni_preavviso = lv_label_create(riga2);
    lv_obj_align(label_giorni_preavviso, LV_ALIGN_RIGHT_MID, -20, 0);
    lv_obj_set_style_text_font(label_giorni_preavviso, &lv_font_montserrat_14, 0);

    // ============================================
    // RIGA 3: GIORNI CRITICO
    // ============================================
    lv_obj_t *riga3 = lv_obj_create(container_principale);
    lv_obj_set_size(riga3, 690, 45);
    lv_obj_align(riga3, LV_ALIGN_TOP_MID, 0, 96);  // Era 110, gap ridotto
    lv_obj_set_style_bg_color(riga3, lv_color_hex(0xF8F9FA), 0);
    lv_obj_set_style_border_width(riga3, 0, 0);
    lv_obj_set_style_radius(riga3, 5, 0);
    lv_obj_set_style_pad_all(riga3, 0, 0);

    lv_obj_t *label_critico_text = lv_label_create(riga3);
    lv_label_set_text(label_critico_text, "Giorni critico:");
    lv_obj_align(label_critico_text, LV_ALIGN_LEFT_MID, 20, 0);
    lv_obj_set_style_text_font(label_critico_text, &lv_font_montserrat_16, 0);

    slider_giorni_critico = lv_slider_create(riga3);
    lv_slider_set_range(slider_giorni_critico, 1, 15);
    lv_obj_set_width(slider_giorni_critico, 220);
    lv_obj_set_height(slider_giorni_critico, 20);
    lv_obj_align(slider_giorni_critico, LV_ALIGN_RIGHT_MID, -140, 0);
    ...
    lv_obj_t *btn_minus_critico, *btn_plus_critico;
    crea_bottoni_plus_minus_estremi(riga3, slider_giorni_critico, &btn_plus_critico, &btn_minus_critico);
    ...
    label_giorni_critico = lv_label_create(riga3);
    lv_obj_align(label_giorni_critico, LV_ALIGN_RIGHT_MID, -20, 0);
    lv_obj_set_style_text_font(label_giorni_critico, &lv_font_montserrat_14, 0);

    // ============================================
    // RIGA 4: FREQUENZA NOTIFICHE
    // ============================================
    lv_obj_t *riga4 = lv_obj_create(container_principale);
    lv_obj_set_size(riga4, 690, 45);
    lv_obj_align(riga4, LV_ALIGN_TOP_MID, 0, 144);  // Era 165, gap ridotto
    lv_obj_set_style_bg_color(riga4, lv_color_white(), 0);
    lv_obj_set_style_border_width(riga4, 0, 0);
    lv_obj_set_style_radius(riga4, 5, 0);
    lv_obj_set_style_pad_all(riga4, 0, 0);

    lv_obj_t *label_freq_text = lv_label_create(riga4);
    lv_label_set_text(label_freq_text, "Frequenza notifiche:");
    lv_obj_align(label_freq_text, LV_ALIGN_LEFT_MID, 20, 0);
    lv_obj_set_style_text_font(label_freq_text, &lv_font_montserrat_16, 0);

    dropdown_frequenza = lv_dropdown_create(riga4);
    lv_dropdown_set_options(dropdown_frequenza, "Ogni avvio\nUna volta al giorno\nUna volta a settimana");
    lv_obj_set_width(dropdown_frequenza, 220);
    lv_obj_align(dropdown_frequenza, LV_ALIGN_RIGHT_MID, -20, 0);
    lv_obj_add_event_cb(dropdown_frequenza, dropdown_frequenza_cb, LV_EVENT_VALUE_CHANGED, NULL);

    // ============================================
    // RIGA 5: STATO ALLARME
    // ============================================
    lv_obj_t *riga5 = lv_obj_create(container_principale);
    lv_obj_set_size(riga5, 690, 55);                // Era 60, ridotto leggermente
    lv_obj_align(riga5, LV_ALIGN_TOP_MID, 0, 192);  // Era 220, gap ridotto
    lv_obj_set_style_bg_color(riga5, lv_color_hex(0xE3F2FD), 0);
    lv_obj_set_style_border_width(riga5, 1, 0);
    lv_obj_set_style_border_color(riga5, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_radius(riga5, 5, 0);
    lv_obj_set_style_pad_all(riga5, 5, 0);

    label_stato_allarme = lv_label_create(riga5);
    lv_obj_align(label_stato_allarme, LV_ALIGN_LEFT_MID, 15, 0);
    ...
    lv_obj_set_width(label_stato_allarme, 660);
    ...
    lv_obj_align(btn_configura_filtro, LV_ALIGN_BOTTOM_LEFT, 40, -10);
    ...
    lv_obj_align(btn_snooze_clear, LV_ALIGN_BOTTOM_MID, -70, -10);
    ...
    lv_obj_align(btn_salva, LV_ALIGN_BOTTOM_RIGHT, -40, -10);
...
void controlla_auto_ritorno() {
  ...
  if (transizione_in_corso) return;  // AGGIUNGI QUESTA RIGA
