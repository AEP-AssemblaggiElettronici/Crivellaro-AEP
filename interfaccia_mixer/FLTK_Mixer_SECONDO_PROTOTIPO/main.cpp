...
#define DIMENSIONE_PULSANTE_CANALE 40
...
Fl_Window *finestra = new Fl_Window (1120, 380);
BottoneMixer *bottoni[NUMERO_BOTTONI];
BottoneBanco *bottoniBanco[NUMERO_BOTTONI_CANALI];
Fl_Button *bottoneCarica = new Fl_Button(900, SPAZIO_VERTICALE_1, 100, 40, "LOAD");
Fl_Button *bottoneSalva = new Fl_Button(900, SPAZIO_VERTICALE_2 , 100, 40, "SAVE");
...
int main (int argc, char ** argv)
{
  ...
  {
    bottoni[i] = i < 12 ?
      new BottoneMixer((i + 1) * DIMENSIONE_PULSANTE_MIXER, SPAZIO_VERTICALE_1, DIMENSIONE_PULSANTE_MIXER, DIMENSIONE_PULSANTE_MIXER, etichette[i]) :
      new BottoneMixer(((i - 12) + 1) * DIMENSIONE_PULSANTE_MIXER, SPAZIO_VERTICALE_2, DIMENSIONE_PULSANTE_MIXER, DIMENSIONE_PULSANTE_MIXER, etichette[i]);
  }

  for (int i = 0; i < NUMERO_BOTTONI_CANALI; i++) // istanziamento bottoni banco
  {
    bottoniBanco[i] = i < 26 ?
      new BottoneBanco((i + 1) * DIMENSIONE_PULSANTE_CANALE, SPAZIO_VERTICALE_BANCO_1, DIMENSIONE_PULSANTE_CANALE, DIMENSIONE_PULSANTE_CANALE, i + 1) :
      new BottoneBanco(((i - 26) + 1) * DIMENSIONE_PULSANTE_CANALE, SPAZIO_VERTICALE_BANCO_2, DIMENSIONE_PULSANTE_CANALE, DIMENSIONE_PULSANTE_CANALE, i + 1);
