...
{
    unsigned int conteggioInviato = 0;
    int conteggio = 0;
    ...
    while (millis() < conteggioFinale)
    {
        ...
        if (statoAttuale != statoPrecedente)
            if (statoAttuale == 1) // solo la transizione basso-alto ? conteggiata
                conteggio++;
    ...
    if (conteggioInviato == 65534)
        Serial.println("Pluviometro non presente in questo ciclo");
    else
    {
        Serial.print("Pluviometro in questo ciclo: ");
