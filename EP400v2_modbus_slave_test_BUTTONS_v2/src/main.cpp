...
void blinky_show()
{ // piccolo lampeggio prima di mostrare sui led i valori di ModBus ID o di sensibilità
...
void loop()
{
  // every 4 seconds, sensor are gathering data:
...
/*   char buff[10];
  sprintf(buff, "%d %d %d", discreteInputs[0], discreteInputs[1], discreteInputs[2]);
  Serial.println(buff); */
  wdt_reset();

  if (!discreteInputs[0] && !discreteInputs[1] && !discreteInputs[2])
  { // everything OK
    alarm = false;
    if (millis() % 4000 == 0)
    {
