...
// ============================================
// CONFIGURAZIONE AUTO-RITORNO SCHERMATA PRINCIPALE
// ============================================
#define AUTO_RETURN_TIMEOUT_MS 30000  // 30 secondi di inattività
static unsigned long last_touch_time = 0;
static bool auto_return_enabled = false;

// Variabili per il nuovo layout orario
lv_obj_t *label_ore_grande = NULL;
lv_obj_t *label_minuti_grande = NULL;
lv_obj_t *btn_ore_plus = NULL, *btn_ore_minus = NULL;
lv_obj_t *btn_minuti_plus = NULL, *btn_minuti_minus = NULL;
static int current_ore = 12;
static int current_minuti = 0;

// Flag per prevenire click multipli durante transizioni
static bool transizione_in_corso = false;
static unsigned long tempo_ultima_transizione = 0;
#define MIN_INTERVALLO_TRANSIZIONE 500  // ms minimo tra transizioni

// ============================================
// PROTOTIPI FUNZIONI (DICHIARAZIONI ANTICIPATE)
// ============================================
void reset_auto_return_timer();
void enable_auto_return();
void disable_auto_return();
void controlla_auto_ritorno();
static void touch_event_cb(lv_event_t *e);
bool safe_lvgl_operation(std::function<void()> operation, uint32_t timeout_ms = LVGL_TIMEOUT_MS);  // ✅ DEFAULT QUI
bool safe_screen_transition(lv_obj_t *target_screen, lv_scr_load_anim_t anim_type, uint32_t time, bool auto_del);
...
extern const lv_img_dsc_t acqua_fresca, acqua_frizzante, acqua_liscia, bicchiere, bottiglina, bottiglia, freccia_cerchiata, impostazioni_allarme, impostazioni_aspetto, impostazioni_bicchiere, impostazioni_contatore, impostazioni_schermo;
...
struct AlarmSettings {
  uint8_t mesi_dopo_installazione = 1;  // Mesi DOPO installazione per l'allarme (default 1)
...
// Oggetti LVGL per schermata allarmi (SEMPLIFICATA)
lv_obj_t *schermo_allarmi = NULL;
lv_obj_t *slider_giorni_scadenza = NULL;  // UNICO SLIDER
lv_obj_t *label_giorni_scadenza = NULL;   // Mostra il numero
...
void carica_impostazioni_allarmi() {
  ...
  alarm_settings.mesi_dopo_installazione = preferences.getUChar("mesi_dopo", 1);
  alarm_settings.last_notification_day = preferences.getUInt("last_notif", 0);
  preferences.end();
  Serial.printf("Allarmi caricati: mesi_dopo_installazione=%d\n", alarm_settings.mesi_dopo_installazione);
}
...
void salva_impostazioni_allarmi() {
  ...
  preferences.putUChar("mesi_dopo", alarm_settings.mesi_dopo_installazione);
  preferences.putUInt("last_notif", alarm_settings.last_notification_day);

  preferences.end();

  Serial.printf("✅ Salvato: mesi_dopo_installazione=%d\n", alarm_settings.mesi_dopo_installazione);

  // Verifica rilettura
  preferences.begin("alarm_system", false);
  uint8_t verifica = preferences.getUChar("mesi_dopo", 0);
  preferences.end();

  Serial.printf("🔍 Verifica rilettura: %d\n", verifica);

  if (verifica != alarm_settings.mesi_dopo_installazione) {
    Serial.println("❌ ERRORE: Valore salvato non corrisponde!");
  }
}
...
AlarmLevel get_current_alarm_level() {
  ...
  // ✅ NUOVA LOGICA: Allarme scatta X MESI DOPO installazione
  int giorni_allarme = alarm_settings.mesi_dopo_installazione * 30;  // Converti mesi in giorni

  if (giorni_rimanenti < 0) {
    return ALARM_EXPIRED;  // Filtro scaduto
  } else if (giorni_dall_installazione >= giorni_allarme) {
    return ALARM_WARNING;  // Allarme attivo (sono passati X mesi)
  }
...
const char *get_alarm_level_text(AlarmLevel level) {
  switch (level) {
    case ALARM_WARNING: return "?? PREAVVISO";
...
static void slider_giorni_scadenza_cb(lv_event_t *e) {
  lv_obj_t *slider = lv_event_get_target(e);
  alarm_settings.mesi_dopo_installazione = lv_slider_get_value(slider);

  safe_lvgl_operation([&]() {
    static char buf[32];
    snprintf(buf, sizeof(buf), "%d", alarm_settings.mesi_dopo_installazione);
    lv_label_set_text(label_giorni_scadenza, buf);
  });

  // RESET TIMER: Resetta la data ultima notifica
  alarm_settings.last_notification_day = 0;

  salva_impostazioni_allarmi();
  aggiorna_display_allarmi();

  Serial.printf("Mesi dopo installazione aggiornati: %d (timer resettato)\n", alarm_settings.mesi_dopo_installazione);
}
...
void aggiorna_display_allarmi() {
  ...
  safe_lvgl_operation([&]() {
    // Aggiorna slider
    if (slider_giorni_scadenza) {
      lv_slider_set_value(slider_giorni_scadenza, alarm_settings.mesi_dopo_installazione, LV_ANIM_OFF);
    }

    // Aggiorna label numero
    if (label_giorni_scadenza) {
      static char buf[16];
      snprintf(buf, sizeof(buf), "%d", alarm_settings.mesi_dopo_installazione);
      lv_label_set_text(label_giorni_scadenza, buf);
    }
    ...
    if (giorni_installazione_filtro == 0) {
      lv_label_set_text(label_stato_allarme, "⚠️ Filtro non ancora installato");
      lv_obj_set_style_text_color(label_stato_allarme, lv_color_hex(0x888888), 0);
      return;
    }

    int giorni_dall_installazione = calcola_giorni_dall_installazione();
    float mesi_dall_installazione = calcola_mesi_dall_installazione();
    int giorni_limite = limite_mesi_filtro * 30;
    int giorni_rimanenti = giorni_limite - giorni_dall_installazione;
    int giorni_allarme = alarm_settings.mesi_dopo_installazione * 30;
    int giorni_mancanti_allarme = giorni_allarme - giorni_dall_installazione;
    ...
    if (level == ALARM_NONE) {
      snprintf(buffer, sizeof(buffer),
               ...
               "Allarme scatterà tra: %d giorni (%.1f mesi)\n"
               "(dopo %d mesi dall'installazione)",
               mesi_dall_installazione,
               giorni_mancanti_allarme > 0 ? giorni_mancanti_allarme : 0,
               (giorni_mancanti_allarme > 0 ? giorni_mancanti_allarme : 0) / 30.0f,
               alarm_settings.mesi_dopo_installazione);
      ...
      snprintf(buffer, sizeof(buffer),
               "⚠️ ALLARME ATTIVO\n\n"
               "Mesi dall'installazione: %.1f\n"
               "Soglia allarme: %d mesi\n"
               "Mesi alla scadenza filtro: %.1f\n\n"
               "⚡ Notifica ad ogni avvio del sistema",
               mesi_dall_installazione,
               alarm_settings.mesi_dopo_installazione,
               giorni_rimanenti / 30.0f);
      ...
      snprintf(buffer, sizeof(buffer),
               ...
               "Mesi dall'installazione: %.1f\n"
               "Scaduto da: %.1f mesi\n\n"
               "⚠️ SOSTITUIRE IMMEDIATAMENTE IL FILTRO!",
               mesi_dall_installazione,
               -giorni_rimanenti / 30.0f);
    }
...
public:
  ...
  void check_memory() {
    ...
    if (free_heap < MEMORY_WARNING_THRESHOLD) {
      Serial.printf("?? MEMORIA BASSA: %d bytes (min: %d)\n", free_heap, min_free_heap);
    }
  }
...
// ============================================
// CLASSE GESTIONE I2C CON RESET PERIODICO
// ============================================
...
private:
  ...
  uint32_t reset_interval_ms = 300000;        // Reset ogni 5 minuti
  uint32_t health_check_interval_ms = 30000;  // Health check ogni 30 secondi
...
public:
  void init() {
    ...
    Serial.println("✅ I2C Manager inizializzato");
  }

  // Reset soft del bus I2C
  bool soft_reset() {
    Serial.println("🔄 I2C Soft Reset...");

    // Prova a fare unlock del mutex
    if (i2c_mutex) {
      for (int i = 0; i < 3; i++) {
    ...
    Serial.println("✅ I2C Soft Reset completato");
    return true;
  }

  // Reset completo del bus I2C (più aggressivo)
  // Reset completo del bus I2C (più aggressivo)
  bool hard_reset() {
    Serial.println("⚠️ I2C Hard Reset...");

    // 1. Unlock mutex multiplo
    if (i2c_mutex) {
      for (int i = 0; i < 5; i++) {
        xSemaphoreGive(i2c_mutex);
      }
    ...
    Serial.println("✅ I2C Hard Reset completato");
    return true;
  }

  // Chiamata periodica nel loop
  void update() {
    ...
    // Reset periodico preventivo
    if (now - last_reset > reset_interval_ms) {
      soft_reset();
    }
  }

  // Verifica salute del bus
  void check_health() {
    // Calcola error rate
    float error_rate = (total_operations > 0) ? (float)error_count / total_operations : 0;

    if (error_rate > 0.1) {  // Più del 10% di errori
      Serial.printf("⚠️ I2C Error rate alto: %.1f%%\n", error_rate * 100);
      bus_healthy = false;
      hard_reset();
    } else if (!bus_healthy) {
      Serial.println("✅ I2C Bus tornato healthy");
      bus_healthy = true;
    }
  }

  // Registra operazione
  void log_operation(bool success) {
    total_operations++;
    if (!success) {
      error_count++;

      // Se troppi errori consecutivi, reset
      if (error_count > 5) {
        Serial.println("⚠️ Troppi errori I2C consecutivi, reset...");
        hard_reset();
      }
    }
  }

  // Imposta intervallo reset (per debug)
  void set_reset_interval(uint32_t ms) {
    reset_interval_ms = ms;
    Serial.printf("I2C reset interval: %lu ms\n", ms);
  }
  ...
  void print_stats() {
    ...
    Serial.printf("📊 I2C Stats: %lu operazioni, %lu errori (%.1f%%), status: %s\n",
                  total_operations, error_count, error_rate * 100,
                  bus_healthy ? "OK" : "DEGRADED");
  }
...
public:
  ...
  static bool init() {
    ...
    Serial.println("?? Debug e valvole condivideranno la stessa seriale!");
#else
    ...
    if (sendResetCommand()) {
      ...
      Serial.println("?? Valvole non rispondono - modalità simulazione");
      return false;
    }
  }
...
// ============================================
// FUNZIONI HELPER THREAD-SAFE (IMPLEMENTAZIONI)
// ============================================

bool safe_lvgl_operation(std::function<void()> operation, uint32_t timeout_ms) {
  unsigned long start_time = millis();

  if (lvgl_port_lock(timeout_ms)) {
    unsigned long wait_time = millis() - start_time;

    if (wait_time > 100) {
      Serial.printf("⚠️ LVGL lock lento: %lu ms\n", wait_time);
    }
    ...
    try {
      ...
      Serial.println("❌ Eccezione in operazione LVGL!");
      lvgl_port_unlock();
      return false;
    }
  }

  Serial.printf("❌ LVGL lock timeout! (tentato per %lu ms)\n", timeout_ms);
  return false;
}
...
// NOTA: safe_screen_transition viene definita PIÙ TARDI
// perché usa enable_auto_return() che è definita dopo
...
lv_obj_t *crea_schermata_impostazioni_standard(const char *titolo) {
  ...
  lv_obj_align(freccia_indietro, LV_ALIGN_TOP_LEFT, 25, 30);
  lv_obj_add_event_cb(
    freccia_indietro, [](lv_event_t *e) {
      safe_screen_transition(schermo2, LV_SCR_LOAD_ANIM_FADE_IN, 150, false);
    },
...
public:
  ...
  static bool creaOggettoLVGL(uint16_t *rgb565_data, uint32_t width, uint32_t height) {
    ...
    bool success = safe_lvgl_operation([&]() {
      ...
      lv_obj_align(img_logo_obj, LV_ALIGN_TOP_MID, 0, 20);
  ...
  static bool aggiornaAutomaticoDaSD() {
    ...
    if (!SDCardManager::exists(LOGO_SD_PATH)) {
      Serial.println("?? Nessun logo.bmp su SD");
      return false;
    }
    ...
    if (SPIFFS.exists(LOGO_SPIFFS_PATH)) {
      ...
      Serial.println("??? Vecchio logo rimosso da SPIFFS");
    }
...
// ============================================
// FUNZIONI PER GESTIONE TEMPI EROGAZIONE
// ============================================

void carica_tempi_erogazione() {
  Serial.println("Caricamento tempi erogazione...");
  preferences.begin("erogazione", false);

  tempoContenitore[0] = preferences.getUChar("tempo_bicch", 10);   // Default 10s
  tempoContenitore[1] = preferences.getUChar("tempo_bott", 20);    // Default 20s
  tempoContenitore[2] = preferences.getUChar("tempo_bottig", 30);  // Default 30s

  preferences.end();

  Serial.printf("Tempi erogazione caricati: Bicchiere=%ds, Bottiglina=%ds, Bottiglia=%ds\n",
                tempoContenitore[0], tempoContenitore[1], tempoContenitore[2]);
}

void salva_tempi_erogazione() {
  Serial.println("Salvataggio tempi erogazione...");
  preferences.begin("erogazione", false);

  preferences.putUChar("tempo_bicch", tempoContenitore[0]);
  preferences.putUChar("tempo_bott", tempoContenitore[1]);
  preferences.putUChar("tempo_bottig", tempoContenitore[2]);

  preferences.end();

  Serial.printf("✅ Tempi salvati: Bicchiere=%ds, Bottiglina=%ds, Bottiglia=%ds\n",
                tempoContenitore[0], tempoContenitore[1], tempoContenitore[2]);
}
...
uint32_t ottieni_giorni_correnti() {
  time_t now;
  time(&now);
  uint32_t giorni = (uint32_t)(now / (24 * 60 * 60));

  Serial.printf("📅 ottieni_giorni_correnti(): now=%lu, giorni=%lu\n", (unsigned long)now, giorni);
...
void reset_filtro() {
  Serial.println("=== RESET FILTRO ===");

  uint32_t giorni_prima = giorni_installazione_filtro;
  giorni_installazione_filtro = ottieni_giorni_correnti();
  filtro_scaduto_notificato = false;

  Serial.printf("Prima: %lu, Dopo: %lu\n", giorni_prima, giorni_installazione_filtro);

  salva_dati_filtro();

  Serial.println("✅ Filtro resettato - nuova installazione registrata");
  Serial.printf("📊 Verifica: giorni_installazione_filtro = %lu\n", giorni_installazione_filtro);
}
...
void aggiorna_display_filtro() {
  ...
  safe_lvgl_operation([&]() {
    ...
    if (filtro_scaduto()) {
      snprintf(buffer, sizeof(buffer), "Stato: ?? SCADUTO (da %d giorni)", -giorni_rimanenti);
...
static void btn_plus_cb(lv_event_t *e) {
  ...
  if (slider) {
    ...
    if (current_val < max_val) {
      ...
      // ✅ AGGIUNGI DEBUG
      Serial.printf("➕ Bottone +: valore %d -> %d\n", current_val, current_val + 1);
    }
  }
}
...
void mostra_notifica_allarme_avanzata() {
  ...
  safe_lvgl_operation([&]() {
    ...
    switch (level) {
      case ALARM_WARNING:
        strcpy(titolo, "?? PREAVVISO FILTRO");
        snprintf(messaggio, sizeof(messaggio),
                 "Il filtro si avvicina alla scadenza!\n\n"
                 "?? Mesi dall'installazione: %.1f / %d\n"
                 "? Giorni rimanenti: %d\n\n"
                 "??? Pianifica la sostituzione del filtro\n"
...
void callback_bottone_impostazioni_erogazione(lv_event_t *evento) {
  // AGGIUNGI CONTROLLO TRANSIZIONE
  if (transizione_in_corso) {
    Serial.println("Transizione in corso, click ignorato");
    return;
  }
  ...
  safe_lvgl_operation([&]() {
    ...
    // Disabilita auto-ritorno per questa schermata
    disable_auto_return();
    lv_obj_add_event_cb(schermo_impostazioni_erogazione, touch_event_cb, LV_EVENT_PRESSED, NULL);

    // Container principale per tabella erogazione
    lv_obj_t *container_tabella = lv_obj_create(schermo_impostazioni_erogazione);
    lv_obj_set_size(container_tabella, 700, 250);
    lv_obj_align(container_tabella, LV_ALIGN_CENTER, 0, 50);
    ...
    lv_obj_set_style_pad_all(container_tabella, 10, 0);

    // Header tabella
    lv_obj_t *header_erogazione = lv_obj_create(container_tabella);
    lv_obj_set_size(header_erogazione, 680, 40);
    lv_obj_align(header_erogazione, LV_ALIGN_TOP_MID, 0, 0);
    lv_obj_set_style_bg_color(header_erogazione, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_radius(header_erogazione, 5, 0);
    lv_obj_set_style_border_width(header_erogazione, 0, 0);

    // Headers colonne
    lv_obj_t *h1 = lv_label_create(header_erogazione);
    lv_label_set_text(h1, "Contenitore");
    lv_obj_align(h1, LV_ALIGN_LEFT_MID, 20, 0);
    lv_obj_set_style_text_color(h1, lv_color_white(), 0);
    lv_obj_set_style_text_font(h1, &lv_font_montserrat_16, 0);

    lv_obj_t *h2 = lv_label_create(header_erogazione);
    lv_label_set_text(h2, "Tempo (secondi)");
    lv_obj_align(h2, LV_ALIGN_LEFT_MID, 200, 0);
    lv_obj_set_style_text_color(h2, lv_color_white(), 0);
    lv_obj_set_style_text_font(h2, &lv_font_montserrat_16, 0);

    lv_obj_t *h3 = lv_label_create(header_erogazione);
    lv_label_set_text(h3, "Controlli");
    lv_obj_align(h3, LV_ALIGN_LEFT_MID, 450, 0);
    lv_obj_set_style_text_color(h3, lv_color_white(), 0);
    lv_obj_set_style_text_font(h3, &lv_font_montserrat_16, 0);

    // Struttura dati per le righe
    struct {
      const char *nome;
      uint8_t *tempo_ptr;
      lv_obj_t **slider_ptr;
      ...
      { "Bicchiere", &tempoContenitore[0], &slider_bicchiere_erogazione, &label_bicchiere_erogazione },
      { "Bottiglina", &tempoContenitore[1], &slider_bottiglina_erogazione, &label_bottiglina_erogazione },
      { "Bottiglia", &tempoContenitore[2], &slider_bottiglia_erogazione, &label_bottiglia_erogazione }
    };

    for (int i = 0; i < 3; i++) {
      // Container riga
      lv_obj_t *riga = lv_obj_create(container_tabella);
      lv_obj_set_size(riga, 680, 50);
      lv_obj_align(riga, LV_ALIGN_TOP_MID, 0, 50 + (i * 55));
      lv_obj_set_style_bg_color(riga, i % 2 ? lv_color_hex(0xF8F9FA) : lv_color_white(), 0);
      lv_obj_set_style_border_width(riga, 0, 0);
      lv_obj_set_style_radius(riga, 0, 0);
      lv_obj_set_style_pad_all(riga, 5, 0);

      // Colonna 1: Nome contenitore
      lv_obj_t *label_nome = lv_label_create(riga);
      lv_label_set_text(label_nome, righe_erog[i].nome);
      lv_obj_align(label_nome, LV_ALIGN_LEFT_MID, 15, 0);
      lv_obj_set_style_text_font(label_nome, &lv_font_montserrat_16, 0);
      lv_obj_set_style_text_color(label_nome, lv_color_hex(0x333333), 0);

      // Colonna 2: Slider tempo
      *(righe_erog[i].slider_ptr) = lv_slider_create(riga);
      lv_slider_set_range(*(righe_erog[i].slider_ptr), 0, 60);
      lv_slider_set_value(*(righe_erog[i].slider_ptr), *(righe_erog[i].tempo_ptr), LV_ANIM_OFF);
      lv_obj_set_width(*(righe_erog[i].slider_ptr), 180);
      lv_obj_set_height(*(righe_erog[i].slider_ptr), 20);
      lv_obj_align(*(righe_erog[i].slider_ptr), LV_ALIGN_LEFT_MID, 200, 0);
      lv_obj_set_style_bg_color(*(righe_erog[i].slider_ptr), lv_color_hex(0x0F73B5), LV_PART_INDICATOR);
      lv_obj_add_event_cb(*(righe_erog[i].slider_ptr), slider_event_erogazione_cb, LV_EVENT_VALUE_CHANGED, NULL);

      // Colonna 3: Pulsanti +/-
      lv_obj_t *btn_minus = lv_btn_create(riga);
      lv_obj_set_size(btn_minus, 40, 30);
      lv_obj_align(btn_minus, LV_ALIGN_LEFT_MID, 450, 0);
      lv_obj_t *label_minus = lv_label_create(btn_minus);
      lv_label_set_text(label_minus, "-");
      lv_obj_center(label_minus);
      lv_obj_set_style_bg_color(btn_minus, lv_color_hex(0x0F73B5), 0);
      lv_obj_set_style_text_font(label_minus, &lv_font_montserrat_20, 0);

      lv_obj_t *btn_plus = lv_btn_create(riga);
      lv_obj_set_size(btn_plus, 40, 30);
      lv_obj_align(btn_plus, LV_ALIGN_LEFT_MID, 500, 0);
      lv_obj_t *label_plus = lv_label_create(btn_plus);
      lv_label_set_text(label_plus, "+");
      lv_obj_center(label_plus);
      lv_obj_set_style_bg_color(btn_plus, lv_color_hex(0x0F73B5), 0);
      lv_obj_set_style_text_font(label_plus, &lv_font_montserrat_20, 0);

      // Imposta user data per i pulsanti
      lv_obj_set_user_data(btn_plus, *(righe_erog[i].slider_ptr));
      lv_obj_set_user_data(btn_minus, *(righe_erog[i].slider_ptr));
      ...
      // Label valore
      *(righe_erog[i].label_ptr) = lv_label_create(riga);
      lv_obj_align(*(righe_erog[i].label_ptr), LV_ALIGN_LEFT_MID, 570, 0);
      lv_obj_set_style_text_font(*(righe_erog[i].label_ptr), &lv_font_montserrat_14, 0);
      lv_obj_set_style_text_color(*(righe_erog[i].label_ptr), lv_color_hex(0x666666), 0);

      // Aggiorna label iniziale
      static char buf[16];
      snprintf(buf, sizeof(buf), "%ds", *(righe_erog[i].tempo_ptr));
      lv_label_set_text(*(righe_erog[i].label_ptr), buf);
    }

    // Bottone Salva
    lv_obj_t *btn_salva = lv_btn_create(schermo_impostazioni_erogazione);
    lv_obj_set_size(btn_salva, 120, 50);
    lv_obj_align(btn_salva, LV_ALIGN_BOTTOM_MID, 0, -5);
    ...
    lv_obj_add_event_cb(
      btn_salva, [](lv_event_t *e) {
        Serial.println("💾 Click bottone Salva erogazione");

        // Salva i tempi
        salva_tempi_erogazione();

        // Torna a schermo2
        safe_screen_transition(schermo2, LV_SCR_LOAD_ANIM_FADE_IN, 150, false);

        Serial.println("✅ Ritornato a schermo2");
      },
      LV_EVENT_CLICKED, NULL);
...
void callback_bottone_impostazioni_allarme(lv_event_t *evento) {
  ...
  safe_lvgl_operation([&]() {
    ...
    disable_auto_return();
    lv_obj_add_event_cb(schermo_allarmi, touch_event_cb, LV_EVENT_PRESSED, NULL);

    // ============================================
    // CONTENITORE PRINCIPALE (COMPATTO)
    // ============================================
    lv_obj_t *container_principale = lv_obj_create(schermo_allarmi);
    lv_obj_set_size(container_principale, 700, 250);
    lv_obj_align(container_principale, LV_ALIGN_CENTER, 0, 30);
    lv_obj_set_style_bg_color(container_principale, lv_color_white(), 0);
    lv_obj_set_style_border_color(container_principale, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_border_width(container_principale, 2, 0);
    lv_obj_set_style_radius(container_principale, 8, 0);
    lv_obj_set_style_pad_all(container_principale, 15, 0);

    // ============================================
    // TITOLO SLIDER
    // ============================================
    lv_obj_t *label_titolo_slider = lv_label_create(container_principale);
    lv_label_set_text(label_titolo_slider, "Mesi dopo installazione per l'allarme:");
    lv_obj_align(label_titolo_slider, LV_ALIGN_TOP_LEFT, 0, 10);
    lv_obj_set_style_text_font(label_titolo_slider, &lv_font_montserrat_18, 0);
    lv_obj_set_style_text_color(label_titolo_slider, lv_color_hex(0x333333), 0);

    // ============================================
    // CONTAINER SLIDER + CONTROLLI
    // ============================================
    lv_obj_t *container_slider = lv_obj_create(container_principale);
    lv_obj_set_size(container_slider, 670, 60);
    lv_obj_align(container_slider, LV_ALIGN_TOP_MID, 0, 50);
    lv_obj_set_style_bg_opa(container_slider, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(container_slider, 0, 0);
    lv_obj_set_style_pad_all(container_slider, 0, 0);

    // Slider
    slider_giorni_scadenza = lv_slider_create(container_slider);
    lv_slider_set_range(slider_giorni_scadenza, 1, 12);  // ✅ Da 1 a 12 mesi
    lv_slider_set_value(slider_giorni_scadenza, alarm_settings.mesi_dopo_installazione, LV_ANIM_OFF);
    lv_obj_set_width(slider_giorni_scadenza, 450);
    lv_obj_set_height(slider_giorni_scadenza, 25);
    lv_obj_align(slider_giorni_scadenza, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_bg_color(slider_giorni_scadenza, lv_color_hex(0x0F73B5), LV_PART_INDICATOR);
    lv_obj_add_event_cb(slider_giorni_scadenza, slider_giorni_scadenza_cb, LV_EVENT_VALUE_CHANGED, NULL);

    // Bottoni +/-
    lv_obj_t *btn_minus, *btn_plus;
    crea_bottoni_plus_minus_estremi(container_slider, slider_giorni_scadenza, &btn_plus, &btn_minus);
    lv_obj_add_event_cb(btn_plus, btn_plus_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_add_event_cb(btn_minus, btn_minus_cb, LV_EVENT_CLICKED, NULL);

    // Label numero giorni (A DESTRA)
    label_giorni_scadenza = lv_label_create(container_slider);
    lv_obj_align(label_giorni_scadenza, LV_ALIGN_RIGHT_MID, -20, 0);
    lv_obj_set_style_text_font(label_giorni_scadenza, &lv_font_montserrat_26, 0);
    lv_obj_set_style_text_color(label_giorni_scadenza, lv_color_hex(0x0F73B5), 0);

    static char buf_init[16];
    snprintf(buf_init, sizeof(buf_init), "%d", alarm_settings.mesi_dopo_installazione);
    lv_label_set_text(label_giorni_scadenza, buf_init);

    // ============================================
    // STATO ALLARME
    // ============================================
    lv_obj_t *container_stato = lv_obj_create(container_principale);
    lv_obj_set_size(container_stato, 670, 70);
    lv_obj_align(container_stato, LV_ALIGN_BOTTOM_MID, 0, -10);
    lv_obj_set_style_bg_color(container_stato, lv_color_hex(0xE3F2FD), 0);
    lv_obj_set_style_border_width(container_stato, 1, 0);
    lv_obj_set_style_border_color(container_stato, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_radius(container_stato, 5, 0);
    lv_obj_set_style_pad_all(container_stato, 10, 0);

    label_stato_allarme = lv_label_create(container_stato);
    lv_obj_align(label_stato_allarme, LV_ALIGN_LEFT_MID, 10, 0);
    lv_obj_set_style_text_font(label_stato_allarme, &lv_font_montserrat_14, 0);
    lv_label_set_long_mode(label_stato_allarme, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(label_stato_allarme, 640);

    // ============================================
    // BOTTONE INSTALLA FILTRO (IN BASSO)
    // ============================================
    lv_obj_t *btn_installa_filtro = lv_btn_create(schermo_allarmi);
    lv_obj_set_size(btn_installa_filtro, 200, 50);
    lv_obj_align(btn_installa_filtro, LV_ALIGN_BOTTOM_LEFT, 60, -10);
    lv_obj_t *label_installa = lv_label_create(btn_installa_filtro);
    lv_label_set_text(label_installa, "Installa Nuovo Filtro");
    lv_obj_center(label_installa);
    lv_obj_set_style_bg_color(btn_installa_filtro, lv_color_hex(0x00AA00), 0);
    lv_obj_set_style_text_font(label_installa, &lv_font_montserrat_14, 0);

    lv_obj_add_event_cb(
      btn_installa_filtro, [](lv_event_t *e) {
        Serial.println("🔧 Click Installa Nuovo Filtro");

        // 1. Reset filtro (registra nuova installazione)
        reset_filtro();
        Serial.printf("✅ Filtro resettato: giorni_installazione=%lu\n", giorni_installazione_filtro);

        // 2. Reset timer notifiche
        alarm_settings.last_notification_day = 0;
        salva_impostazioni_allarmi();

        // 3. Aggiorna display DOPO il reset
        aggiorna_display_allarmi();

        // 4. Mostra conferma
        safe_lvgl_operation([&]() {
          static char msg[200];
          int giorni_al_prossimo_allarme = (alarm_settings.mesi_dopo_installazione * 30);

          snprintf(msg, sizeof(msg),
                   "✅ Nuovo filtro installato!\n\n"
                   "📅 Data installazione: OGGI\n"
                   "⏱️ Timer allarme resettato\n\n"
                   "🔔 Prossimo allarme tra:\n"
                   "%d giorni (%d mesi)",
                   giorni_al_prossimo_allarme,
                   alarm_settings.mesi_dopo_installazione);

          lv_obj_t *success_box = lv_msgbox_create(NULL, "Filtro Installato", msg, NULL, true);
          lv_obj_center(success_box);
        });
      },
      LV_EVENT_CLICKED, NULL);

    // Bottone Salva (torna indietro)
    lv_obj_t *btn_salva = lv_btn_create(schermo_allarmi);
    lv_obj_set_size(btn_salva, 120, 50);
    lv_obj_align(btn_salva, LV_ALIGN_BOTTOM_RIGHT, -60, -10);
    lv_obj_t *label_salva = lv_label_create(btn_salva);
    lv_label_set_text(label_salva, "Chiudi");
    lv_obj_center(label_salva);
    lv_obj_set_style_bg_color(btn_salva, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_text_font(label_salva, &lv_font_montserrat_14, 0);
        ...
        bool sd_init_success = safe_i2c_operation([&]() -> esp_err_t {
          ...
          if (result) {
            ...
            if (cardType != CARD_NONE && totalBytes > 0) {
              ...
              Serial.printf("   ?? Card rilevata ma TotalBytes = %llu\n", totalBytes);
              SD.end();
              delay(100);
            }
          }
...
// ============================================
// SAFE_SCREEN_TRANSITION (definita qui perché usa auto-return)
// ============================================
...
bool safe_screen_transition(lv_obj_t *target_screen, lv_scr_load_anim_t anim_type, uint32_t time, bool auto_del) {
  ...
  // Debounce
  if (now - tempo_ultima_transizione < MIN_INTERVALLO_TRANSIZIONE) {
    Serial.println("⚠️ Transizione ignorata (debounce)");
    return false;
  }

  // Blocca se già in transizione
  if (transizione_in_corso) {
    Serial.println("⚠️ Transizione già in corso, operazione annullata");
    return false;
  }

  transizione_in_corso = true;
  tempo_ultima_transizione = now;

  // Disabilita temporaneamente auto-ritorno durante transizione
  bool auto_return_was_enabled = auto_return_enabled;
  disable_auto_return();

  bool success = safe_lvgl_operation([&]() {
    lv_scr_load_anim(target_screen, anim_type, time, 0, auto_del);
  });

  // Aspetta che animazione completi
  delay(time + 50);

  // Ripristina auto-ritorno se necessario
  if (auto_return_was_enabled) {
    enable_auto_return();
  }

  transizione_in_corso = false;

  return success;
}

// ============================================
// SISTEMA AUTO-RITORNO (IMPLEMENTAZIONI)
// ============================================
void reset_auto_return_timer() {
...
void controlla_auto_ritorno() {
  if (!auto_return_enabled) return;
  if (transizione_in_corso) return;

  unsigned long now = millis();
  if (now - last_touch_time > AUTO_RETURN_TIMEOUT_MS) {
    Serial.println("Auto-ritorno alla schermata principale");
    disable_auto_return();
    safe_screen_transition(schermo1, LV_SCR_LOAD_ANIM_FADE_IN, 300, false);
  }
}

// ============================================
// FUNZIONI LAYOUT ORARIO
// ============================================
void aggiorna_display_orario() {
  safe_lvgl_operation([&]() {
    static char buf_ore[8], buf_minuti[8];
    snprintf(buf_ore, sizeof(buf_ore), "%02d", current_ore);
    snprintf(buf_minuti, sizeof(buf_minuti), "%02d", current_minuti);

    if (label_ore_grande) lv_label_set_text(label_ore_grande, buf_ore);
    if (label_minuti_grande) lv_label_set_text(label_minuti_grande, buf_minuti);
  });
}

static void btn_ore_plus_cb(lv_event_t *e) {
  current_ore = (current_ore + 1) % 24;
  aggiorna_display_orario();
  reset_auto_return_timer();
}

static void btn_ore_minus_cb(lv_event_t *e) {
  current_ore = (current_ore - 1 + 24) % 24;
  aggiorna_display_orario();
  reset_auto_return_timer();
}

static void btn_minuti_plus_cb(lv_event_t *e) {
  current_minuti = (current_minuti + 1) % 60;
  aggiorna_display_orario();
  reset_auto_return_timer();
}

static void btn_minuti_minus_cb(lv_event_t *e) {
  current_minuti = (current_minuti - 1 + 60) % 60;
  aggiorna_display_orario();
  reset_auto_return_timer();
}

static void salva_orario_nuovo_cb(lv_event_t *e) {
  time_t now;
  struct tm timeinfo;
  time(&now);
  localtime_r(&now, &timeinfo);

  timeinfo.tm_hour = current_ore;
  timeinfo.tm_min = current_minuti;
  timeinfo.tm_sec = 0;
  ...
  // CORREZIONE: Riattiva auto-ritorno dopo essere tornati a schermo2
  enable_auto_return();
}
...
void setup() {
  ...
  if (wdt_result == ESP_OK) {
    ...
    if (add_result == ESP_OK) {
      ...
      Serial.printf("?? Errore aggiunta task al WDT: %s\n", esp_err_to_name(add_result));
    }
  } else {
    Serial.printf("?? Errore inizializzazione WDT: %s\n", esp_err_to_name(wdt_result));
  }

  // Inizializza monitor sistema
  monitor.init();

  // ✅ INIZIALIZZA I2C MANAGER
  i2c_manager.init();
  i2c_manager.set_reset_interval(300000);  // Reset ogni 5 minuti (300000 ms)
  ...
  if (!safe_lvgl_operation([&]() {
        ...
        // Freccia navigazione verso impostazioni (alto sinistra)
        lv_obj_t *freccia_vai_impostazioni = lv_imgbtn_create(schermo1);
        lv_imgbtn_set_src(freccia_vai_impostazioni, LV_IMGBTN_STATE_RELEASED, NULL, &freccia_cerchiata, NULL);
        lv_obj_set_size(freccia_vai_impostazioni, SIZE_ICON_CERCHIATA, SIZE_ICON_CERCHIATA);
        lv_obj_align(freccia_vai_impostazioni, LV_ALIGN_TOP_LEFT, 25, 30);
        // Ruota la freccia di 180 gradi per puntare a destra
        //lv_obj_set_style_transform_angle(freccia_vai_impostazioni, 1800, 0);  // 1800 = 180 gradi (LVGL usa decimi di grado)
        lv_obj_add_event_cb(
          freccia_vai_impostazioni, [](lv_event_t *e) {
            enable_auto_return();
            safe_screen_transition(schermo2, LV_SCR_LOAD_ANIM_FADE_IN, 150, false);
          },
          LV_EVENT_CLICKED, NULL);
        ...
        lv_obj_align(label_orologio, LV_ALIGN_TOP_RIGHT, -25, 30);
        ...
        // Freccia indietro verso schermata principale
        lv_obj_t *freccia_torna_principale = lv_imgbtn_create(schermo2);
        lv_imgbtn_set_src(freccia_torna_principale, LV_IMGBTN_STATE_RELEASED, NULL, &freccia_cerchiata, NULL);
        lv_obj_set_size(freccia_torna_principale, SIZE_ICON_CERCHIATA, SIZE_ICON_CERCHIATA);
        lv_obj_align(freccia_torna_principale, LV_ALIGN_TOP_LEFT, 25, 30);
        lv_obj_add_event_cb(
          freccia_torna_principale, [](lv_event_t *e) {
            safe_lvgl_operation([&]() {
              disable_auto_return();
...
#define TERZA_COLONNA 560
  ...
  Serial.println("🕐 Inizializzazione RTC...");
  setup_rtc();  // ✅✅✅ FONDAMENTALE!
  ...
  carica_tempi_erogazione();
  Serial.println("✅ Tempi erogazione caricati");
  ...
  if (sd_disponibile) {
    ...
    Serial.println("?? SD card non disponibile - UI funziona comunque");
  }
...
static void gesture_cb(lv_event_t *e) {
  ...
  if (dir == LV_DIR_RIGHT) {
    enable_auto_return();  // NUOVO
    safe_screen_transition(schermo2, LV_SCR_LOAD_ANIM_FADE_IN, 150, false);
  } else if (dir == LV_DIR_LEFT) {
    disable_auto_return();  // NUOVO
    safe_lvgl_operation([&]() {
      lv_scr_load_anim(schermo1, LV_SCR_LOAD_ANIM_MOVE_LEFT, 150, 0, false);
    });
  }
...
void callback_bottone_impostazioni_schermo(lv_event_t *evento) {
  ...
  safe_lvgl_operation([&]() {
    ...
    // Aggiungi callback per eventi di tocco
    lv_obj_add_event_cb(schermo_impostazioni_orologio, touch_event_cb, LV_EVENT_PRESSED, NULL);
    lv_obj_add_event_cb(schermo_impostazioni_orologio, touch_event_cb, LV_EVENT_CLICKED, NULL);

    // Ottieni orario corrente
    time_t now_time = time(NULL);
    struct tm now;
    localtime_r(&now_time, &now);
    current_ore = now.tm_hour;
    current_minuti = now.tm_min;

    // Container principale per orario
    lv_obj_t *container_orario = lv_obj_create(schermo_impostazioni_orologio);
    lv_obj_set_size(container_orario, 500, 200);
    lv_obj_align(container_orario, LV_ALIGN_CENTER, 0, 40);
    lv_obj_set_style_bg_opa(container_orario, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(container_orario, 0, 0);
    lv_obj_set_style_pad_all(container_orario, 0, 0);

    // COLONNA ORE (SINISTRA)
    lv_obj_t *label_ore_title = lv_label_create(container_orario);
    lv_label_set_text(label_ore_title, "ORE");
    lv_obj_align(label_ore_title, LV_ALIGN_TOP_LEFT, 100, 20);
    lv_obj_set_style_text_font(label_ore_title, &lv_font_montserrat_26, 0);
    lv_obj_set_style_text_color(label_ore_title, lv_color_hex(0x0F73B5), 0);

    // Numero ore grande (centrato)
    label_ore_grande = lv_label_create(container_orario);
    lv_obj_align(label_ore_grande, LV_ALIGN_TOP_LEFT, 100, 50);
    lv_obj_set_style_text_font(label_ore_grande, &lv_font_montserrat_48, 0);
    lv_obj_set_style_text_color(label_ore_grande, lv_color_hex(0x333333), 0);

    // Pulsante + ore
    btn_ore_plus = lv_btn_create(container_orario);
    lv_obj_set_size(btn_ore_plus, 60, 40);
    lv_obj_align(btn_ore_plus, LV_ALIGN_TOP_LEFT, 50, 120);
    lv_obj_t *label_ore_plus = lv_label_create(btn_ore_plus);
    lv_label_set_text(label_ore_plus, "+");
    lv_obj_center(label_ore_plus);
    lv_obj_set_style_bg_color(btn_ore_plus, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_text_font(label_ore_plus, &lv_font_montserrat_26, 0);
    lv_obj_add_event_cb(btn_ore_plus, btn_ore_plus_cb, LV_EVENT_CLICKED, NULL);

    // Pulsante - ore
    btn_ore_minus = lv_btn_create(container_orario);
    lv_obj_set_size(btn_ore_minus, 60, 40);
    lv_obj_align(btn_ore_minus, LV_ALIGN_TOP_LEFT, 120, 120);
    lv_obj_t *label_ore_minus = lv_label_create(btn_ore_minus);
    lv_label_set_text(label_ore_minus, "-");
    lv_obj_center(label_ore_minus);
    lv_obj_set_style_bg_color(btn_ore_minus, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_text_font(label_ore_minus, &lv_font_montserrat_26, 0);
    lv_obj_add_event_cb(btn_ore_minus, btn_ore_minus_cb, LV_EVENT_CLICKED, NULL);

    // SEPARATORE CENTRALE
    lv_obj_t *separatore_due_punti = lv_label_create(container_orario);
    lv_label_set_text(separatore_due_punti, ":");
    lv_obj_align(separatore_due_punti, LV_ALIGN_TOP_MID, 0, 60);
    lv_obj_set_style_text_font(separatore_due_punti, &lv_font_montserrat_48, 0);
    lv_obj_set_style_text_color(separatore_due_punti, lv_color_hex(0x0F73B5), 0);

    // COLONNA MINUTI (DESTRA)
    lv_obj_t *label_minuti_title = lv_label_create(container_orario);
    lv_label_set_text(label_minuti_title, "MINUTI");
    lv_obj_align(label_minuti_title, LV_ALIGN_TOP_RIGHT, -100, 20);
    lv_obj_set_style_text_font(label_minuti_title, &lv_font_montserrat_26, 0);
    lv_obj_set_style_text_color(label_minuti_title, lv_color_hex(0x0F73B5), 0);

    // Numero minuti grande
    label_minuti_grande = lv_label_create(container_orario);
    lv_obj_align(label_minuti_grande, LV_ALIGN_TOP_RIGHT, -100, 50);
    lv_obj_set_style_text_font(label_minuti_grande, &lv_font_montserrat_48, 0);
    lv_obj_set_style_text_color(label_minuti_grande, lv_color_hex(0x333333), 0);

    // Pulsante + minuti
    btn_minuti_plus = lv_btn_create(container_orario);
    lv_obj_set_size(btn_minuti_plus, 60, 40);
    lv_obj_align(btn_minuti_plus, LV_ALIGN_TOP_RIGHT, -130, 120);
    lv_obj_t *label_minuti_plus = lv_label_create(btn_minuti_plus);
    lv_label_set_text(label_minuti_plus, "+");
    lv_obj_center(label_minuti_plus);
    lv_obj_set_style_bg_color(btn_minuti_plus, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_text_font(label_minuti_plus, &lv_font_montserrat_26, 0);
    lv_obj_add_event_cb(btn_minuti_plus, btn_minuti_plus_cb, LV_EVENT_CLICKED, NULL);

    // Pulsante - minuti
    btn_minuti_minus = lv_btn_create(container_orario);
    lv_obj_set_size(btn_minuti_minus, 60, 40);
    lv_obj_align(btn_minuti_minus, LV_ALIGN_TOP_RIGHT, -60, 120);
    lv_obj_t *label_minuti_minus = lv_label_create(btn_minuti_minus);
    lv_label_set_text(label_minuti_minus, "-");
    lv_obj_center(label_minuti_minus);
    lv_obj_set_style_bg_color(btn_minuti_minus, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_text_font(label_minuti_minus, &lv_font_montserrat_26, 0);
    lv_obj_add_event_cb(btn_minuti_minus, btn_minuti_minus_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_set_size(btn_salva, 120, 50);
    lv_obj_align(btn_salva, LV_ALIGN_BOTTOM_MID, 0, -20);
    lv_obj_t *label_btn = lv_label_create(btn_salva);
    lv_label_set_text(label_btn, "Salva");
    lv_obj_center(label_btn);
    lv_obj_set_style_bg_color(btn_salva, lv_color_hex(0x0F73B5), 0);
    lv_obj_add_event_cb(btn_salva, salva_orario_nuovo_cb, LV_EVENT_CLICKED, NULL);
...
void callback_bottone_impostazioni_contatore(lv_event_t *evento) {
  ...
  safe_lvgl_operation([&]() {
    ...
    // Disabilita auto-ritorno per questa schermata
    disable_auto_return();
    lv_obj_add_event_cb(schermo_contatore, touch_event_cb, LV_EVENT_PRESSED, NULL);
    ...
    lv_obj_align(container_tabella, LV_ALIGN_CENTER, 0, 30);  // Posizionato sotto la linea Y=150
    lv_obj_set_style_bg_color(container_tabella, lv_color_white(), 0);
    lv_obj_set_style_border_color(container_tabella, lv_color_hex(0x0F73B5), 0);
    lv_obj_set_style_border_width(container_tabella, 2, 0);
    lv_obj_set_style_radius(container_tabella, 8, 0);
...
void loop() {
  ...
  // ✅ AGGIORNA I2C MANAGER
  i2c_manager.update();

  // NUOVO: Controllo auto-ritorno
  controlla_auto_ritorno();
  ...
  if (now - last_operations[3] > 300000) {
    monitor.print_stats();
    i2c_manager.print_stats();  // ✅ AGGIUNGI statistiche I2C
