...
lv_obj_t *slider_secondi, *label_secondi = NULL;
...
public:
  ...
  static bool creaOggettoLVGL(uint16_t *rgb565_data, uint32_t width, uint32_t height) {
    ...
    bool success = safe_lvgl_operation([&]() {
      ...
      lv_obj_align(img_logo_obj, LV_ALIGN_TOP_MID, 0, 10);
...
void callback_menu_logo(lv_event_t *evento) {
  safe_lvgl_operation([&]() {
    // Crea schermata menu logo
    lv_obj_t *schermo_logo = lv_obj_create(NULL);

    // Freccia indietro
    lv_obj_t *freccia_indietro = lv_imgbtn_create(schermo_logo);
    ...
    // Titolo
    lv_obj_t *titolo = lv_label_create(schermo_logo);
    lv_label_set_text(titolo, "GESTIONE LOGO");
    lv_obj_align(titolo, LV_ALIGN_TOP_MID, 0, 30);
    ...
    lv_obj_align(info_logo, LV_ALIGN_CENTER, 0, -60);
    lv_obj_set_style_text_font(info_logo, &lv_font_montserrat_16, 0);

    // Bottone "Mostra Logo"
    lv_obj_t *btn_mostra = lv_btn_create(schermo_logo);
    lv_obj_set_size(btn_mostra, 180, 50);
    lv_obj_align(btn_mostra, LV_ALIGN_CENTER, 0, -10);
    lv_obj_t *label_mostra = lv_label_create(btn_mostra);
    lv_label_set_text(label_mostra, "Mostra Info");
    ...
    // Bottone "Aggiorna da SD"
    lv_obj_t *btn_aggiorna = lv_btn_create(schermo_logo);
    lv_obj_set_size(btn_aggiorna, 180, 50);
    lv_obj_align(btn_aggiorna, LV_ALIGN_CENTER, 0, 50);
    lv_obj_t *label_aggiorna = lv_label_create(btn_aggiorna);
    lv_label_set_text(label_aggiorna, "Aggiorna da SD");
    ...
    // Bottone "Rimuovi Logo"
    lv_obj_t *btn_rimuovi = lv_btn_create(schermo_logo);
    lv_obj_set_size(btn_rimuovi, 180, 50);
    lv_obj_align(btn_rimuovi, LV_ALIGN_CENTER, 0, 110);
    lv_obj_t *label_rimuovi = lv_label_create(btn_rimuovi);
    lv_label_set_text(label_rimuovi, "Rimuovi Logo");
    ...
    // Bottone "Test SD" per debug
    lv_obj_t *btn_test_sd = lv_btn_create(schermo_logo);
    lv_obj_set_size(btn_test_sd, 180, 50);
    lv_obj_align(btn_test_sd, LV_ALIGN_CENTER, 0, 170);
    lv_obj_t *label_test_sd = lv_label_create(btn_test_sd);
    lv_label_set_text(label_test_sd, "Test SD Card");
    ...
    lv_obj_add_event_cb(
      btn_test_sd, [](lv_event_t *e) {
        ...
        while (file && file_count < 15) {  // Aumentato da 10 a 15
        ...
        // Usa un ritardo prima di mostrare il dialog per evitare timeout LVGL
        delay(100);

        // Prova senza safe_lvgl_operation per evitare timeout
...
// Callback per test comunicazione valvole (da aggiungere a un bottone) - RIMOSSA
// void callback_test_comunicazione_valvole(lv_event_t *evento) - FUNZIONE RIMOSSA
...
void callback_bottone_impostazioni_contatore(lv_event_t *evento) {
  safe_lvgl_operation([&]() {
    ...
    schermo_contatore = lv_obj_create(NULL);

    // Freccia indietro
    lv_obj_t *freccia_indietro = lv_imgbtn_create(schermo_contatore);
    lv_imgbtn_set_src(freccia_indietro, LV_IMGBTN_STATE_RELEASED, NULL, &freccia_cerchiata, NULL);
    lv_obj_set_size(freccia_indietro, SIZE_ICON_CERCHIATA, SIZE_ICON_CERCHIATA);
    lv_obj_align(freccia_indietro, LV_ALIGN_TOP_LEFT, 20, 20);
    lv_obj_add_event_cb(
    ...
    // Titolo
    lv_obj_t *titolo = lv_label_create(schermo_contatore);
    lv_label_set_text(titolo, "CONTATORE EROGAZIONI");
    lv_obj_align(titolo, LV_ALIGN_TOP_MID, 0, 30);
    lv_obj_set_style_text_font(titolo, &lv_font_montserrat_30, 0);

    // Label statistiche
    label_litri = lv_label_create(schermo_contatore);
    lv_obj_align(label_litri, LV_ALIGN_CENTER, 0, -40);
    lv_obj_set_style_text_font(label_litri, &lv_font_montserrat_16, 0);

    label_bottiglie = lv_label_create(schermo_contatore);
    lv_obj_align(label_bottiglie, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_text_font(label_bottiglie, &lv_font_montserrat_16, 0);

    label_denaro = lv_label_create(schermo_contatore);
    lv_obj_align(label_denaro, LV_ALIGN_CENTER, 0, 40);
    lv_obj_set_style_text_font(label_denaro, &lv_font_montserrat_16, 0);

    label_ultima_erogazione = lv_label_create(schermo_contatore);
    lv_obj_align(label_ultima_erogazione, LV_ALIGN_CENTER, 0, 80);
    lv_obj_set_style_text_font(label_ultima_erogazione, &lv_font_montserrat_12, 0);
    ...
    lv_obj_align(btn_dettagli, LV_ALIGN_BOTTOM_RIGHT, -20, -20);
    ...
    lv_obj_add_event_cb(
      btn_dettagli, [](lv_event_t *e) {
        safe_lvgl_operation([&]() {
          ...
          lv_obj_t *mbox = lv_msgbox_create(NULL, "Info Calcoli", dettagli, NULL, true);
          lv_obj_center(mbox);
        });
      },
      LV_EVENT_CLICKED, NULL);
...
void callback_bottone_impostazioni_allarme(lv_event_t *evento) {
  safe_lvgl_operation([&]() {
    safe_screen_delete(schermo_filtri);
    schermo_filtri = lv_obj_create(NULL);

    // Freccia indietro
    lv_obj_t *freccia_indietro = lv_imgbtn_create(schermo_filtri);
    lv_imgbtn_set_src(freccia_indietro, LV_IMGBTN_STATE_RELEASED, NULL, &freccia_cerchiata, NULL);
    lv_obj_set_size(freccia_indietro, SIZE_ICON_CERCHIATA, SIZE_ICON_CERCHIATA);
    lv_obj_align(freccia_indietro, LV_ALIGN_TOP_LEFT, 20, 20);
    lv_obj_add_event_cb(
    ...
    // Titolo
    lv_obj_t *titolo = lv_label_create(schermo_filtri);
    lv_label_set_text(titolo, "GESTIONE FILTRI");
    lv_obj_align(titolo, LV_ALIGN_TOP_MID, 0, 30);
    lv_obj_set_style_text_font(titolo, &lv_font_montserrat_30, 0);

    // Label informazioni filtro
    label_giorni_filtro = lv_label_create(schermo_filtri);
    lv_obj_align(label_giorni_filtro, LV_ALIGN_CENTER, 0, -80);
    lv_obj_set_style_text_font(label_giorni_filtro, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_align(label_giorni_filtro, LV_TEXT_ALIGN_CENTER, 0);

    // Label stato filtro
    label_stato_filtro = lv_label_create(schermo_filtri);
    lv_obj_align(label_stato_filtro, LV_ALIGN_CENTER, 0, -40);
    lv_obj_set_style_text_font(label_stato_filtro, &lv_font_montserrat_16, 0);
    lv_obj_set_style_text_align(label_stato_filtro, LV_TEXT_ALIGN_CENTER, 0);

    // Slider per limite mesi
    lv_obj_t *label_slider = lv_label_create(schermo_filtri);
    lv_label_set_text(label_slider, "Durata filtro (mesi):");
    lv_obj_align(label_slider, LV_ALIGN_CENTER, 0, 10);

    slider_limite_mesi = lv_slider_create(schermo_filtri);
    lv_slider_set_range(slider_limite_mesi, 1, 24);
    lv_obj_set_width(slider_limite_mesi, 250);
    lv_obj_align(slider_limite_mesi, LV_ALIGN_CENTER, 0, 40);
    lv_obj_add_event_cb(slider_limite_mesi, slider_limite_mesi_cb, LV_EVENT_VALUE_CHANGED, NULL);

    label_limite_mesi = lv_label_create(schermo_filtri);
    lv_obj_align(label_limite_mesi, LV_ALIGN_CENTER, 0, 70);

    // Switch per controllo automatico
    lv_obj_t *label_switch = lv_label_create(schermo_filtri);
    lv_label_set_text(label_switch, "Controllo automatico:");
    lv_obj_align(label_switch, LV_ALIGN_CENTER, -50, 110);

    switch_controllo_automatico = lv_switch_create(schermo_filtri);
    lv_obj_align(switch_controllo_automatico, LV_ALIGN_CENTER, 50, 110);
    lv_obj_add_event_cb(switch_controllo_automatico, switch_controllo_cb, LV_EVENT_VALUE_CHANGED, NULL);

    // Bottoni azione
    lv_obj_t *btn_installato = lv_btn_create(schermo_filtri);
    lv_obj_set_size(btn_installato, 140, 50);
    lv_obj_align(btn_installato, LV_ALIGN_BOTTOM_LEFT, 20, -20);
    lv_obj_t *label_installato = lv_label_create(btn_installato);
    lv_label_set_text(label_installato, "Filtro Nuovo");
    lv_obj_center(label_installato);
    lv_obj_set_style_bg_color(btn_installato, lv_color_hex(0x00AA00), 0);

    lv_obj_add_event_cb(
      btn_installato, [](lv_event_t *e) {
        safe_lvgl_operation([&]() {
          static const char *btns[] = { "Annulla", "Conferma", "" };
          lv_obj_t *mbox = lv_msgbox_create(NULL, "Nuovo Filtro",
                                            "Confermi l'installazione di un nuovo filtro?\n"
                                            "Questo resetterà il contatore dei giorni.",
                                            btns, true);
          lv_obj_center(mbox);
    ...
    lv_obj_t *btn_test = lv_btn_create(schermo_filtri);
    lv_obj_set_size(btn_test, 120, 50);
    lv_obj_align(btn_test, LV_ALIGN_BOTTOM_MID, 0, -20);
    lv_obj_t *label_test = lv_label_create(btn_test);
    lv_label_set_text(label_test, "Test Alert");
    lv_obj_center(label_test);
    lv_obj_set_style_bg_color(btn_test, lv_color_hex(0x0088FF), 0);

    lv_obj_add_event_cb(
      btn_test, [](lv_event_t *e) {
        if (giorni_installazione_filtro == 0) {
          mostra_errore_sd("Installa prima un filtro!");
        } else {
    ...
    // Bottone "Salva"
    lv_obj_t *btn_salva = lv_btn_create(schermo_filtri);
    lv_obj_set_size(btn_salva, 100, 50);
    lv_obj_align(btn_salva, LV_ALIGN_BOTTOM_RIGHT, -20, -20);
...
void setup() {
  ...
  if (!safe_lvgl_operation([&]() {
        ...
        // creazione cerchio di selezione
        ...
        lv_obj_set_style_bg_color(cerchio_selezione, lv_color_hex(0x00AEEF), 0);
        lv_obj_set_style_bg_opa(cerchio_selezione, LV_OPA_30, 0);
        ...
        lv_obj_align(label_orologio, LV_ALIGN_TOP_RIGHT, -10, 10);
...
#define TERZA_COLONNA 560
        ...
        // bottoni impostazioni seconda riga
        ...
        lv_obj_align(bottone_impostazioni_contatore, LV_ALIGN_TOP_LEFT, PRIMA_COLONNA, SECONDA_RIGA);
  ...
  // Collegamento callback (RIMOSSO bottone aspetto)
...
static void animazione_completata_cb(lv_anim_t *a) {
  ...
  // SOSTITUITA: ferma_erogazione_valvole() -> ferma_erogazione_valvole()
...
void erogazione(uint8_t tempo, lv_obj_t *target, uint8_t tipo_contenitore) {
  // SOSTITUITA: avvia_erogazione_valvole(tipoAcqua) -> avvia_erogazione_valvole(tipoAcqua)
...
void callback_bottone_impostazioni_schermo(lv_event_t *evento) {
  safe_lvgl_operation([&]() {
    schermo_impostazioni_orologio = crea_schermata_generica("IMPOSTA ORARIO");
    ...
    lv_obj_set_width(slider_ore, 200);
    lv_obj_align(slider_ore, LV_ALIGN_CENTER, 0, -60);
    ...
    lv_obj_align_to(label_ore, slider_ore, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    ...
    lv_obj_set_width(slider_minuti, 200);
    lv_obj_align(slider_minuti, LV_ALIGN_CENTER, 0, 0);
    ...
    lv_obj_align_to(label_minuti, slider_minuti, LV_ALIGN_OUT_RIGHT_MID, 10, 0);

    slider_secondi = lv_slider_create(schermo_impostazioni_orologio);
    lv_slider_set_range(slider_secondi, 0, 59);
    lv_slider_set_value(slider_secondi, now.tm_sec, LV_ANIM_OFF);
    lv_obj_set_width(slider_secondi, 200);
    lv_obj_align(slider_secondi, LV_ALIGN_CENTER, 0, 60);
    lv_obj_add_event_cb(slider_secondi, slider_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    label_secondi = lv_label_create(schermo_impostazioni_orologio);
    lv_obj_align_to(label_secondi, slider_secondi, LV_ALIGN_OUT_RIGHT_MID, 10, 0);

    aggiorna_label_orario();

    lv_obj_t *btn_salva = lv_btn_create(schermo_impostazioni_orologio);
    lv_obj_align(btn_salva, LV_ALIGN_CENTER, 0, 120);
...
static void salva_orario_cb(lv_event_t *e) {
  ...
  int secondi = lv_slider_get_value(slider_secondi);
...
void aggiorna_label_orario() {
  safe_lvgl_operation([&]() {
    ...
    snprintf(buf, sizeof(buf), "Secondi: %d", lv_slider_get_value(slider_secondi));
    lv_label_set_text(label_secondi, buf);
  });
}

void callback_bottone_impostazioni_erogazione(lv_event_t *evento) {
  safe_lvgl_operation([&]() {
    schermo_impostazioni_erogazione = lv_obj_create(NULL);

    lv_obj_t *freccia_indietro = lv_imgbtn_create(schermo_impostazioni_erogazione);
    lv_imgbtn_set_src(freccia_indietro, LV_IMGBTN_STATE_RELEASED, NULL, &freccia_cerchiata, NULL);
    lv_obj_set_size(freccia_indietro, SIZE_ICON_CERCHIATA, SIZE_ICON_CERCHIATA);
    lv_obj_align(freccia_indietro, LV_ALIGN_TOP_LEFT, 20, 20);
    lv_obj_add_event_cb(
    ...
    lv_obj_t *label = lv_label_create(schermo_impostazioni_erogazione);
    lv_label_set_text(label, "Tempo di Erogazione");
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 20);

    slider_bicchiere_erogazione = lv_slider_create(schermo_impostazioni_erogazione);
    lv_slider_set_range(slider_bicchiere_erogazione, 0, 128);
    lv_slider_set_value(slider_bicchiere_erogazione, tempoContenitore[0], LV_ANIM_OFF);
    lv_obj_set_width(slider_bicchiere_erogazione, 300);
    lv_obj_align(slider_bicchiere_erogazione, LV_ALIGN_CENTER, 0, -60);
    ...
    lv_obj_align_to(label_bicchiere_erogazione, slider_bicchiere_erogazione, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    {
      static char buf[32];
    ...
    lv_slider_set_range(slider_bottiglina_erogazione, 0, 128);
    lv_slider_set_value(slider_bottiglina_erogazione, tempoContenitore[1], LV_ANIM_OFF);
    lv_obj_set_width(slider_bottiglina_erogazione, 300);
    lv_obj_align(slider_bottiglina_erogazione, LV_ALIGN_CENTER, 0, 0);
    ...
    lv_obj_align_to(label_bottiglina_erogazione, slider_bottiglina_erogazione, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    {
      static char buf[32];
    ...
    lv_slider_set_range(slider_bottiglia_erogazione, 0, 128);
    lv_slider_set_value(slider_bottiglia_erogazione, tempoContenitore[2], LV_ANIM_OFF);
    lv_obj_set_width(slider_bottiglia_erogazione, 300);
    lv_obj_align(slider_bottiglia_erogazione, LV_ALIGN_CENTER, 0, 60);
    ...
    lv_obj_align_to(label_bottiglia_erogazione, slider_bottiglia_erogazione, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
    {
      static char buf[32];
